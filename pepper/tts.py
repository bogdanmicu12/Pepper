...
def _replace_match(replacement):
    def _fn(match):
        text = match.group(0)
        if text.isupper():
...
def normalize_text(text):
    replacements = [
        (r"\b(?:paper|papa|peper)\w*\b", "pepper"),
        (r"\bspa\b", "Spar(supermarket)"),
    ]
    for pattern, repl in replacements:
        text = re.sub(pattern, _replace_match(repl), text, flags=re.IGNORECASE)
    return text

def set_vocal_params(tts, speed=82.0, volume=0.58, pitch=0.97):
    try:
...
def main():
    ...
    tts.say(normalize_text(args.say))
